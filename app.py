...
# ============================================================
# UPDATED RUL LOGIC ONLY
# ============================================================
def estimate_rul(smoothed_error, anomaly_threshold):
    """
    Updated so RUL does not stay fixed at MAX_RUL_HOURS.

    New behavior:
    - failed region -> 0
    - insufficient history -> RUL follows health
    - stable / improving -> RUL follows health
    - degrading -> RUL is reduced below health depending on slope
    """
    failure_threshold = anomaly_threshold * FAILURE_MULTIPLIER
    ...
    # Not enough trend history yet: use health-based RUL
    if slope is None:
        rul = health
        return float(clamp(rul, 0.0, MAX_RUL_HOURS)), "insufficient_history"

    # Stable or improving: still use health-based RUL, not fixed 100
    if slope <= 0:
        rul = health
        return float(clamp(rul, 0.0, MAX_RUL_HOURS)), "stable"

    # Degrading: reduce RUL below health according to degradation rate
    # slope is in error-units per hour
    trend_penalty = min(max(slope * 0.05, 0.0), 0.8)

    # Distance factor: closer to failure => lower RUL
    distance_ratio = (failure_threshold - smoothed_error) / max((failure_threshold - anomaly_threshold), 1e-9)
    distance_ratio = clamp(distance_ratio, 0.0, 1.0)

    # Combine present health and remaining distance, then apply trend penalty
    base_rul = (0.7 * health) + (0.3 * distance_ratio * MAX_RUL_HOURS)
    rul = base_rul * (1.0 - trend_penalty)
