...
def estimate_rul(smoothed_error, anomaly_threshold):
    """
    OPTION 2: Hybrid RUL model

    Logic:
    - Health is the base RUL estimate
    - If not enough history, use Health directly
    - If stable/improving, use Health directly
    - If degrading, reduce RUL below Health using a trend penalty
    """
    ...
    # Not enough history yet -> use health-based RUL
    if slope is None:
        rul = health
        return float(clamp(rul, 0.0, MAX_RUL_HOURS)), "insufficient_history"

    # Stable or improving -> use health-based RUL
    if slope <= 0:
        rul = health
        return float(clamp(rul, 0.0, MAX_RUL_HOURS)), "stable"

    # Positive degradation trend -> apply penalty
    # Tune 0.05 if you want stronger/weaker RUL sensitivity
    trend_penalty = min(max(slope * 0.05, 0.0), 0.8)
    rul = health * (1.0 - trend_penalty)
