...
MIN_READINGS_FOR_RUL = 20   # minimum points needed before estimating
    ...
    try:
        ...
        if len(health_history) >= MIN_READINGS_FOR_RUL:
            ...
            try:
                ...
                # slope threshold: -0.1 %/hour means at least 2.4% drop per day
                if slope < -0.1:
                    ...
                    # Health stable or improving — RUL is not applicable
                    # Reset last_rul so dashboard shows N/A, not a stale number
                    last_rul = None
                    rul = -1.0
