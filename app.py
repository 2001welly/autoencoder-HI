...
def estimate_rul(smoothed_error, anomaly_threshold):
    """
    Updated RUL logic:
    - no longer stays fixed at 100 for stable/insufficient history
    - falls back to health-based RUL in those cases
    - still uses degradation trend when slope is positive
    """
    failure_threshold = anomaly_threshold * FAILURE_MULTIPLIER
    ...
    # Not enough history yet: use current health as approximate RUL
    if slope is None:
        rul = health
        return float(clamp(rul, 0.0, MAX_RUL_HOURS)), "insufficient_history"

    # Stable or improving: do not force max RUL, use health-based RUL
    if slope <= 0:
        rul = health
        return float(clamp(rul, 0.0, MAX_RUL_HOURS)), "stable"

    # Degrading: combine health with a trend penalty
