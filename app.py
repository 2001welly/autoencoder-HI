...
# Blending factors for Option 3 RUL model
...
def estimate_rul(smoothed_error, anomaly_threshold):
    """
    Option 3: health-weighted distance-to-failure RUL model

    RUL depends on:
    - distance to failure threshold
    - degradation trend (slope)
    - health reserve as a stabilizer

    This prevents RUL from simply being equal to health, while still making it stable.
    """
    failure_threshold = anomaly_threshold * FAILURE_MULTIPLIER
