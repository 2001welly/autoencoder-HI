...
# ============================================================
# SIMPLE CORS SUPPORT FOR DASHBOARD API CALLS
# ============================================================
@app.after_request
...
# ============================================================
# SHIFT CONTROLLER EMAIL ALERT SETTINGS
# ============================================================
EMAIL_ALERTS_ENABLED = os.getenv("EMAIL_ALERTS_ENABLED", "true").strip().lower() in {"1", "true", "yes", "on"}
EMAIL_ALERT_COOLDOWN_SECONDS = int(os.getenv("EMAIL_ALERT_COOLDOWN_SECONDS", "600"))
EMAIL_ALERT_URGENCY_LEVELS = {
    item.strip().upper()
    for item in os.getenv(
        "EMAIL_ALERT_URGENCY_LEVELS",
        "WARNING,PLAN_MAINTENANCE,URGENT,CRITICAL"
    ).split(",")
    if item.strip()
}
...
SMTP_FROM_NAME = os.getenv("SMTP_FROM_NAME", "Predictive Maintenance Alert").strip()
...
current_shift_controller_email = None
current_shift_started_at = None
last_email_alert_time = None
last_email_alert_status = "No email alert has been sent yet."
last_email_alert_details = {
    "sent": False,
    "reason": "no_alert_sent_yet",
}
...
# ============================================================
# SHIFT CONTROLLER EMAIL ALERT HELPERS
# ============================================================
EMAIL_PATTERN = re.compile(r"^[^@\s]+@[^@\s]+\.[^@\s]+$")


def is_valid_email(email):
    return bool(email and EMAIL_PATTERN.match(str(email).strip()))
...
def get_shift_alert_state():
    return {
        "email_alerts_enabled": bool(EMAIL_ALERTS_ENABLED),
        "smtp_configured": bool(smtp_is_configured()),
        "current_controller_email": current_shift_controller_email,
        "shift_started_at": int(current_shift_started_at) if current_shift_started_at else None,
        "cooldown_seconds": int(EMAIL_ALERT_COOLDOWN_SECONDS),
        "alert_levels": sorted(list(EMAIL_ALERT_URGENCY_LEVELS)),
        "last_alert_time": int(last_email_alert_time) if last_email_alert_time else None,
        "last_alert_status": last_email_alert_status,
        "last_alert_details": last_email_alert_details,
    }


def build_shift_alert_subject(analysis):
    urgency = str(analysis.get("urgency_level") or analysis.get("status") or "ALERT").upper()
    main_cause = str(analysis.get("main_cause") or analysis.get("dominant_feature") or "machine")
    return f"[{urgency}] Predictive maintenance alert - {main_cause}"


def build_shift_alert_body(analysis):
    latest = analysis.get("latest_values") or {}
    actions = analysis.get("prescription_actions") or []
    actions_text = "\n".join([f"- {action}" for action in actions]) if actions else "- No action listed"
    timestamp = analysis.get("analysis_timestamp") or int(time.time())

    return f"""Predictive Maintenance Alert

An abnormal machine condition has been detected by the monitoring system.

Status: {analysis.get('status', '--')}
Urgency level: {analysis.get('urgency_level', '--')}
Maintenance priority score: {analysis.get('maintenance_priority_score', '--')}
Health: {analysis.get('health', '--')}%
Estimated RUL: {analysis.get('rul', '--')} hours
Main cause: {analysis.get('main_cause', '--')}
Operating region: {analysis.get('operating_region', '--')}

Latest sensor readings:
- Current: {latest.get('current', analysis.get('current', '--'))} A
- Temperature: {latest.get('temperature', analysis.get('temperature', '--'))} °C
- Vibration: {latest.get('vibration', analysis.get('vibration', '--'))}

Prescription:
{analysis.get('prescription_title', '--')}

Recommended actions:
{actions_text}

Reason:
{analysis.get('prescription_reason', '--')}

Alert time: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(timestamp))}

This message was sent automatically to the active shift controller registered on the web dashboard.
"""


def send_shift_alert_email(to_email, analysis):
    if not EMAIL_ALERTS_ENABLED:
        return False, "Email alerts are disabled."

    if not smtp_is_configured():
        return False, "SMTP settings are missing. Set SMTP_HOST, SMTP_PORT, and SMTP_FROM_EMAIL."

    if not is_valid_email(to_email):
        return False, "The shift controller email address is invalid."

    message = EmailMessage()
    message["Subject"] = build_shift_alert_subject(analysis)
    message["From"] = f"{SMTP_FROM_NAME} <{SMTP_FROM_EMAIL}>" if SMTP_FROM_NAME else SMTP_FROM_EMAIL
    message["To"] = to_email
    message.set_content(build_shift_alert_body(analysis))
    ...
    try:
        ...
        traceback.print_exc()
        return False, f"Email alert failed: {str(exc)}"


def maybe_send_shift_email_alert(analysis):
    global last_email_alert_time, last_email_alert_status, last_email_alert_details

    urgency = str(analysis.get("urgency_level") or "").upper()
    now = time.time()
    ...
    details = {
        ...
        "sent": False,
        "reason": "not_checked",
        "controller_email": current_shift_controller_email,
        "smtp_configured": bool(smtp_is_configured()),
    ...
    if not EMAIL_ALERTS_ENABLED:
        details["reason"] = "email_alerts_disabled"
        last_email_alert_details = details
        return details

    if urgency not in EMAIL_ALERT_URGENCY_LEVELS:
        details["reason"] = "urgency_not_in_email_alert_levels"
        last_email_alert_details = details
        return details

    if not current_shift_controller_email:
        details["reason"] = "no_shift_controller_email_set"
        last_email_alert_details = details
        return details

    if last_email_alert_time is not None:
        elapsed = now - float(last_email_alert_time)
        remaining = EMAIL_ALERT_COOLDOWN_SECONDS - elapsed
        if remaining > 0:
            details["reason"] = "cooldown_active"
            details["cooldown_remaining_seconds"] = int(max(0, remaining))
            last_email_alert_details = details
            return details

    sent, message = send_shift_alert_email(current_shift_controller_email, analysis)
    details["sent"] = bool(sent)
    details["reason"] = "sent" if sent else "send_failed"
    details["message"] = message
    details["last_alert_time"] = int(now) if sent else details["last_alert_time"]

    if sent:
        last_email_alert_time = now
        last_email_alert_status = f"Email alert sent to {current_shift_controller_email} at {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(now))}."
    else:
        last_email_alert_status = message

    details["last_alert_status"] = last_email_alert_status
    last_email_alert_details = details
    return details
    ...
    return jsonify(
        {
            ...
            "current_shift_controller_email": current_shift_controller_email,
            "email_alert_cooldown_seconds": EMAIL_ALERT_COOLDOWN_SECONDS,
            "email_alert_levels": sorted(list(EMAIL_ALERT_URGENCY_LEVELS)),
        }
    )
...
@app.route("/shift-controller", methods=["GET", "POST", "OPTIONS"])
def shift_controller():
    global current_shift_controller_email, current_shift_started_at, last_email_alert_status

    if request.method == "OPTIONS":
        return ("", 204)

    if request.method == "GET":
        return jsonify(get_shift_alert_state()), 200
    ...
    current_shift_controller_email = email
    current_shift_started_at = time.time()
    last_email_alert_status = "Shift controller email saved. Waiting for an alert condition."

    state = get_shift_alert_state()
    state["ok"] = True
    state["message"] = f"Shift controller email saved: {email}"
...
def batch_predict():
    ...
    try:
        ...
        response["email_alert"] = maybe_send_shift_email_alert(response)
